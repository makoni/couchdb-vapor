...
public func buildApplication(_ arguments: some AppArguments) async throws -> some ApplicationProtocol {
	...
	let couchDBService = CouchDBService(client: couchDBClient)
	let router = buildRouter(couchDBClient: couchDBClient)
...
/// Build router
func buildRouter(couchDBClient: CouchDBClient) -> Router<AppRequestContext> {
	...
	// Add database routes
	let databaseController = DatabaseController(couchDBClient: couchDBClient)
	databaseController.addRoutes(to: router)
